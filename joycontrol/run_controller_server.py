#!/usr/bin/env python3
#referenced from https://github.com/mart1nro/joycontrol
...
async def handle_tcp_server(reader, writer):
    ...
    while True:
        ...
        t0 = time.time()
        timeSinceLastCmd = t0 - t1
        if not data:
            break
        msg = data.decode()
        if msg == "u":
            await button_push(g_controller_state, 'up')
        elif msg == "d":
            await button_push(g_controller_state, 'down')
        elif msg == "l":
            await button_push(g_controller_state, 'left')
        elif msg == "r":
            ...
            await button_push(g_controller_state, 'r')
        elif msg == "2":
            button_state = g_controller_state.button_state
            button_state.set_button('l')
            button_state.set_button('r')
            await g_controller_state.send()
        ...
        t1 = time.time()
        cmdDeltaTime = t1 - t0
        logging.info(f'msg {msg} {timeSinceLastCmd} {cmdDeltaTime}')
...
async def _main(args):
    ...
    with utils.get_output(path=args.log, default=None) as capture_file:
        ...
        g_controller_state = controller_state
...
if __name__ == '__main__':
    ...
    tasks = []
    tasks.append(asyncio.ensure_future(_main(args)))
    tasks.append(asyncio.ensure_future(asyncio.start_server(handle_tcp_server, '127.0.0.1', 8080, loop=loop)))
